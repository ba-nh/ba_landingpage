...
문의하기 버튼으로 제출된 **관심 분야**, **이메일**, **기타 문의 사항**(선택)을 내 Google 시트에 저장하는 설정 방법입니다.
...
이제 문의하기 폼에서 제출하면 같은 시트의 **활성 시트**에 일시, 관심분야, 이메일, 기타문의가 한 행씩 추가됩니다.

## 동작 방식

- 랜딩페이지의 문의 폼은 **숨겨진 form**으로 위 웹 앱 URL에 **POST**로 전송합니다.
- 전송은 **iframe**을 사용해 이루어지므로, 브라우저 CORS 제한 없이 동작합니다.
- Apps Script의 `doPost(e)`가 `e.parameter.interest`, `e.parameter.email`, `e.parameter.inquiry`를 받아 시트에 `appendRow([일시, 관심분야, 이메일, 기타문의])`로 저장합니다.
